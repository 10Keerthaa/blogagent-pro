...
export const DashboardProvider = ({ children }: { children: ReactNode }) => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            const d = await api.generateContent({ prompt, keywords: keywords.join(', ') });
            const contentWithLinks = applySitemapLinks(d.content);
            setPreview({ title: d.title, meta: d.metaDesc, content: contentWithLinks, imageUrl: d.imageUrl });
            setDescription(d.metaDesc);
        } catch (e: any) { setError(e.message); }
    };
    ...
    const handleApplyFeedback = async () => {
        setError(null);
        try {
            const d = await api.generateContent({ prompt, keywords: keywords.join(', '), feedback });
            const contentWithLinks = applySitemapLinks(d.content);
            setPreview({ title: d.title, meta: d.metaDesc, content: contentWithLinks, imageUrl: d.imageUrl });
            setDescription(d.metaDesc);
            setFeedback('');
